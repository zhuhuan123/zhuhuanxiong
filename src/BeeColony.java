...
public  class BeeColony {
	...
	double ub = 5.12; /*upper bound of the parameters.
	 lb and ub can be defined as arrays for the problems
	  of which parameters have different bounds*/
	...
	/*Foods is the population of food sources. Each row of Foods matrix 
	 * is a vector holding D parameters to be optimized. 
	...
	/*New solution (neighbour) produced by v_{ij}=x_{ij}+\phi_{ij}*(x_{kj}-x_{ij}) j 
	 * is a randomly chosen parameter and k is a randomlu chosen solution different from i*/
...
//	FunctionCallback function = &sphere;
	static HashSet<Table> o_meansSet = new HashSet<Table>();
	static ArrayList<String> fileList = new ArrayList<String>();
	static ArrayList<String> nodeList = new ArrayList<String>();
	static ArrayList<MInfo> myInfos = new ArrayList<MInfo>();
	static ArrayList<MIFoCount> mIFoCounts = new ArrayList<MIFoCount>();
	public static void Readfile(String filename, int[] int_, int intNum)
			throws Exception {
		
		String strbuff;
		BufferedReader buffer = new BufferedReader(new InputStreamReader(
				new FileInputStream(filename)));
		// strbuff = buffer.readLine(); //
		while (true) {
			strbuff = buffer.readLine();
			if (strbuff == null)
				break;
			fileList.add(strbuff);
		}
		buffer.close();
		int co = 1;
...
//		nodeList.addAll(nodeSet);
		...
		for (String i : fileList) {
			String[] str = i.split(",");
			MIFoCount infor = new MIFoCount();
			for (int j = 0; j < intNum; j++) {
				infor.getAr().add(str[int_[j]]);
			}
			if (!mIFoCounts.contains(infor)) {
				infor.setId(id);
				infor.setCount(1);
				mIFoCounts.add(infor);
				id++;
			} else {
				mIFoCounts.get(mIFoCounts.indexOf(infor)).setCountAddOne();   //inforList   Ԫ���б�ļ���
			}
		}
	}
	/*Fitness function*/
	...
	void init(int index)
	{
	   ...
	   for (j=0;j<D;j++)
	        ...
	        System.out.println(Foods[index][j]);
			solution[j]=Foods[index][j];
			}
		f[index]=calculateFunction(solution);
		System.out.println("f[index]  "+f[index]);
		fitness[index]=CalculateFitness(f[index]);
		System.out.println("fitness[index] "+fitness[index]);
		trial[index]=0;
	}
	  ...
	  /*Employed Bee Phase*/
	   for (i=0;i<FoodNumber;i++)
	       ...
	       // }
	        ...
	        System.out.println(ObjValSol);
	        FitnessSol=CalculateFitness(ObjValSol);
	        System.out.println(FitnessSol); 
	...
	/*determine the food sources whose trial 
	 * counter exceeds the "limit" value. In Basic ABC,
